...
int main()
{
    ...
    printf("Please enter a positive number for conversion to binary numeric system: ");
    do
    {
        scanf("%d", &num);
        if(num < 0)
    ...
    divided = num; //I do not use the num variable because i want to print the inserted number from the user
    while(1)
    {
        modulo = divided % 2;
        Insert(&AList, Node, &FreePtr, NilValue, modulo);
        divided = divided / 2;
        if(divided == 0)
            break;
    }

    printf("Entered Number: %d\nNumber converted in binary numeric system: ", num);
    TraverseLinked(AList, Node);
