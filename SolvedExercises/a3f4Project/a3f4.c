...
#include <ctype.h>
#include <string.h>
#define NumberOfNodes 30    /*όριο μεγέθους της συνδεδεμένης λίστας,
...
int main()
{
    ...
    ListPointer AList, ALISODD, ALISEVEN;
    NodeType Node[NumberOfNodes], Odd[NumberOfNodes], Even[NumberOfNodes];
    ListPointer FreePtr,PredPtr, FreepOdd, FreePEven;
    FILE *infile;
    int num;
    char ch;
    ListPointer Curr ,PredEven, PredOdd;

    infile = fopen("i3f4.dat", "r");
    if(infile == NULL)
    {
        printf("Problem opening file %s", "i3f4.dat");
        exit(1);
    }

    //Create all my lists the initial with all numbers and the odd and even list
    CreateList(&AList);
    CreateList(&ALISODD);
    CreateList(&ALISEVEN);
    ...
    InitializeStoragePool(Odd, &FreepOdd);
    InitializeStoragePool(Even, &FreePEven);
    ...
    printf("Odd List\n");
    printAll(ALISODD, FreepOdd, Odd);
    printf("\n");

    printf("Even List\n");
    printAll(ALISEVEN, FreePEven, Even);
    printf("\n");
    ...
    printf("Odd List\n");
    TraverseLinked(ALISODD, Odd);
    printf("\n");

    printf("Even List\n");
    TraverseLinked(ALISEVEN, Even);
    printf("\n");

    //Read my numbers from a file and put them into the initial list
    while((ch = fgetc(infile)) != EOF)
    {
        if(ch >= '0' && ch <= '9')
        {
            num = atoi(&ch);
            PredPtr = NilValue;
            Insert(&AList, Node, &FreePtr, PredPtr, num);
        }
    }
    ...
    while(Curr != NilValue)
    {
        if(Node[Curr].Data % 2 == 0)
            Insert(&ALISEVEN, Even, &FreePEven, PredEven = NilValue, Node[Curr].Data);
        else
            Insert(&ALISODD, Odd, &FreepOdd, PredOdd = NilValue, Node[Curr].Data);
