        ...
        switch(choice)
        {
            case 1:
                ...
                while(TRUE)
                {
                    ...
                    printf("DWSE THN IDIOTHTA TOU %s (1 = postgraduate student, 2 = PhD student, 3 = Professor,\
4 = External user): ", AnItem.Data.username);

                    do
                    {
                        scanf("%d", &AnItem.Data.role);
                        if(AnItem.Data.role < 1 || AnItem.Data.role > 4)
                            printf("Please enter valid data.\nDWSE THN IDIOTHTA TOU %s (1 = postgraduate student, 2 = PhD student, 3 = Professor,\
4 = External user): ", AnItem.Data.username);
                    ...
                    for(i = 0; i < HMax; i++)
                    {
                        if(HList->HashTable[i] != EndOfList)
                        {
                            ...
                            while(j != EndOfList)
                            {
                                ...
                                HList->List[j].Data.role == 1 ? "postgraduate student" :
                                HList->List[j].Data.role == 2 ? "PhD student" :
                                HList->List[j].Data.role == 3 ? "Professor" :
                                "External user");
                                j = HList->List[j].Link;
                            }
                        }
