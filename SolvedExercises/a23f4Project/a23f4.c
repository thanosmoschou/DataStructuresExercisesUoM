...
int main()
{
    ...
    printf("Question 10\n");
    maxg = FindMaxs(AList, Node, &grstack);

    printf("Max vathmos=%.1f\n", maxg);
    printf("AM me megistp vathmo: ");

    for(i = grstack.Top; i > -1; i--)
    {
        Pop(&grstack, &tmpam);
        printf("%d ", tmpam);
    }
...
float FindMaxs(ListPointer List, NodeType Node[NumberOfNodes], StackType *Stack)
{
    ...
    {
        tmpmax = Node[List].Data.grade;
        CurrPtr = List;

        while(CurrPtr != NilValue)
        {
            if(Node[CurrPtr].Data.grade > tmpmax)
            {
                    CreateStack(Stack);
                    Push(Stack, Node[CurrPtr].Data.AM);
                    tmpmax = Node[CurrPtr].Data.grade;
            }
            else if(Node[CurrPtr].Data.grade == tmpmax)
                Push(Stack, Node[CurrPtr].Data.AM);
