   ...
   int i, SubIndex;
     ...
     printf("========================================\n");
          ...
          for (i=0; i<HMax;i++)
          {
              ...
              {
                   printf("[%d, %s, %d, %d]",HList.List[SubIndex].Data.memberCode,
                          HList.List[SubIndex].Data.memberName, HList.List[SubIndex].Data.gymWage, HList.List[SubIndex].Link);
                   printf(" -> ");
...
void menu(int *choice)
{
    printf("                  MENOY                  \n");
    printf("-------------------------------------------------\n");
    printf("1. EISAGWGH MELOUS\n");
    printf("2. ANAZHTHSH MELOUS\n");
    printf("3. ENHMERWSH OFEILHS MELOUS\n");
    printf("4. DIAGRAFH MELOUS\n");
    printf("5. EMFANISH MELWN (BOH8HTIKH\n");
    printf("6. EXIT\n");
    printf("\nCHOICE: ");
    do
    {
    ...
    while(TRUE)
    {
        ...
        switch(choice)
        {
            case 1:
                ...
                while(TRUE)
                {
                    printf("\n\nDWSE TO ONOMATEPONYMO: ");
                    scanf("%20[^\n]%c", AnItem.Data.memberName, &termch);
                    if(termch != '\n') //fflush gia to stdin
                        while(getchar() != '\n');

                    printf("DWSE TON KWDIKO TOU %s: ", AnItem.Data.memberName);
                    scanf("%d", &AnItem.Data.memberCode);

                    printf("DWSE THN OFEILH TOU %s: ", AnItem.Data.memberName);
                    scanf("%d", &AnItem.Data.gymWage);

                    AnItem.key = AnItem.Data.memberCode;
                    AddRec(HList, AnItem);

                    printf("SYNEXEIA ME THN EISAGWGH MELWN (Y/N)? ");
                    do
                    {
                        scanf(" %c", &decision);
                        while(getchar() != '\n');
            ...
            case 2:
                ...
                while(TRUE)
                {
                    printf("DWSE TON KWDIKO TOU MELOUS POU THES NA ANAZHTHSEIS: ");
                    scanf("%d", &AKey);

                    SearchHashList(*HList, AKey, &Loc, &Pred);

                    if(Loc != EndOfList) //H endiamesa vrethike h stin arxi
                        printf("ONOMATEPWNYMO: %s, KWDIKOS: %d, OFEILH: %d\n", HList->List[Loc].Data.memberName, HList->List[Loc].Data.memberCode,
                               HList->List[Loc].Data.gymWage);
                    else
                        printf("DEN YPARXEI MELOS ME KWDIKO: %d\n", AKey);

                    printf("SYNEXEIA ME THN ANAZHTHSH MELWN (Y/N)? ");
                    do
                    {
                        scanf(" %c", &decision);
                        while(getchar() != '\n');
                        ...
                        if(toupper(decision) != 'Y' && toupper(decision) != 'N')
                            printf("Please enter valid data.\nSYNEXEIA ME THN ANAZHTHSH MELWN (Y/N)? ");
                    }while(toupper(decision) != 'Y' && toupper(decision) != 'N');
            ...
            case 3:
                ...
                while(TRUE)
                {
                    ...
                    {
                        //FoundMember = TRUE;
                        printf("PARAKALW DWSE TO NEO POSO OFEILHS TOU MELOUS %s: ", HList->List[Loc].Data.memberName);
                        do
                        {
                            scanf("%d", &newWage);
                            if(newWage > HList->List[Loc].Data.gymWage)
                                printf("PREPEI H NEA OFEILH NA EINAI MIKROTERI H ISH ME THN PROHGOUMENH...\nPARAKALW DWSE TO NEO POSO OFEILHS TOU MELOUS %s: "
                                       , HList->List[Loc].Data.memberName);
                        }while(newWage > HList->List[Loc].Data.gymWage);

                        HList->List[Loc].Data.gymWage = newWage;
                    }
                    else
                        printf("DEN YPARXEI MELOS ME KWDIKO: %d\n", AKey);
                ...
                while(TRUE)
                {
                    ...
                    /*for(i = 0; i < HMax; i++)
                    {
                        if(HList->HashTable[i] != EndOfList)
                        {
                            ...
                            while(j != EndOfList)
                            {
                                printf("%d:[%d, %s, %d]\n", j, HList->List[j].Data.memberCode, HList->List[j].Data.memberName,
                                       HList->List[j].Data.gymWage);
                                j = HList->List[j].Link;
                            }
                        }
                    }*/
                    ...
                    Print_HashList(*HList);
                    printf("DWSE TON KWDIKO TOU MELOUS POU THES NA DIAGRAPSEIS: ");
                    scanf("%d", &AKey);
                    ...
                    printf("SYNEXEIA ME THN DIAGRAFH MELWN (Y/N)? ");
                    do
                    {
                        scanf(" %c", &decision);
                        while(getchar() != '\n');
                        ...
                        if(toupper(decision) != 'Y' && toupper(decision) != 'N')
                            printf("Please enter valid data.\nSYNEXEIA ME THN DIAGRAFH MELWN (Y/N)? ");
                    }while(toupper(decision) != 'Y' && toupper(decision) != 'N');
