...
int main()
{
    ...
    int leftNodes1 = 0, leftNodes2 = 0;
    BinTreePointer ARoot, BRoot;

  	CreateBST(&ARoot);
  	CreateBST(&BRoot);
    insertionToTrees(&ARoot, &BRoot);
    leftNodes1 = LeftNodeCount(ARoot);
    leftNodes2 = LeftNodeCount(BRoot);
...
void insertionToTrees(BinTreePointer *ARoot, BinTreePointer *BRoot)
{
    char word1[CHARLIM] = {'\0'}, word2[CHARLIM] = {'\0'};
    //char termch;
    int i ;

    strncpy(word1, "ALGORITHM", 10);
    strncpy(word2, "MHTIROGLA", 10);
...
int LeftNodeCount(BinTreePointer Root)
{
    ...
    if(LocPtr != NULL)
    {
        ...
        if(LocPtr->RChild != NULL)
            right1 = LeftNodeCount(LocPtr->RChild);

        if(LocPtr->LChild != NULL)
        {
            leftNodes++;
            left1 = LeftNodeCount(LocPtr->LChild);
        }

        leftNodes += (left1 + right1);
    }
