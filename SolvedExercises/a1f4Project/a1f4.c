...
int main()
{
    ...
    ListPointer FreePtr,PredPtr, elem;
    char ch;
    ...
    while (1)
    {
      ...
      if(!Search(FreePtr, AList, Node, elem, &PredPtr))
        Insert(&AList, Node, &FreePtr, PredPtr, elem);
...
ListPointer *PredPtr)
{
    ...
    {
      if(Item > Node[tmpcurr].Data)
      {
        prev = tmpcurr;
        tmpcurr = Node[tmpcurr].Next;
      }
      else if(Item == Node[tmpcurr].Data)
      {
        *PredPtr = prev;
        return TRUE;
      }
