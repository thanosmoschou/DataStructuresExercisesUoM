...
void Delete_n_element(ListPointer *AList,int posDel, ListPointer PredPtr);
...
int main()
{
    ...
    if(sumNum > 0)
        Filling(&AList, sumNum, PredPtr = NULL);
    ...
    if(!EmptyList(AList))
    {
        ...
        {
            scanf("%d", &posDel);
            if(posDel <= 0  || posDel > sumNum)
                printf("Please give valid data.\n");
        }while(posDel <= 0 || posDel > sumNum);

        printf("Arxiki lista\n");
        LinkedTraverse(AList);

        Delete_n_element(&AList, posDel, PredPtr = NULL);
