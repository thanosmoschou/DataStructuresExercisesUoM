...
#define IDLIM 9
#define PASSWORDLIM 7

typedef struct
{
    char id[IDLIM];
    char password[PASSWORDLIM];
...
void HashListCreation(HashListType *HList);
int foldkey(ListElm user);
void UserSearchingInsertion(HashListType *HList);
...
int main(){
    ...
    HashListCreation(&HList);
    UserSearchingInsertion(&HList);
...
void PrintPinakes(HashListType HList)
{
    ...
    for (i=0;i<HList.Size;i++)
	   printf("%d) %d, %12s, %9s, %4d\n", i, HList.List[i].key, HList.List[i].Data.id, HList.List[i].Data.password, HList.List[i].Link);
	printf("\n");
}
...
void HashListCreation(HashListType *HList)
{
    ...
    ListElm user;
    int line = 1, scanned;
    char backslashR1, backslashR2, termch1, termch2;

    //filename = fopen("I5f6.txt", "r");
    if((filename = fopen("I5f6.txt", "r")) == NULL)
    {
    ...
    while(TRUE) // while(!feof(filename))
    {
        scanned = fscanf(filename, "%8[^\r]%c%c%6[^\r]%c%c", user.Data.id, &backslashR1, &termch1, user.Data.password, &backslashR2, &termch2);

        if(scanned != 6 && scanned != EOF)
        {
            printf("Problem in lines: %d-%d\n", line, line + 1);
            exit(1);
        }
...
int foldkey(ListElm user)
{
    ...
    //printf("%c:%d, %c:%d\n", user.Data.id[0], user.Data.id[0], user.Data.id[strlen(user.Data.id) - 1], user.Data.id[strlen(user.Data.id) - 1]);
    avg = (user.Data.id[0] + user.Data.id[strlen(user.Data.id) - 1]) / 2;
    //printf("avg: %d\n", avg);
    return avg;
}
...
void UserSearchingInsertion(HashListType *HList)
{
    ListElm userInput;
    int Loc, Pred;
    char termch1, decision;

    while(TRUE)
    {
        printf("USERNAME: ");
        scanf("%8[^\n]%c", userInput.Data.id, &termch1);
        if(termch1 != '\n')
            while(getchar() != '\n');

        printf("PASSWORD: ");
        scanf("%6[^\n]%c", userInput.Data.password, &termch1);
        if(termch1 != '\n')
            while(getchar() != '\n');

        userInput.key = foldkey(userInput);

        SearchHashList(*HList, userInput.key, &Loc, &Pred);
        if(Loc != EndOfList)
        {
            if(strncmp(HList->List[Loc].Data.password, userInput.Data.password, strlen(HList->List[Loc].Data.password)) == 0)
        ...
        do
        {
            scanf("%c%c", &decision, &termch1);
            //An dwsei perissoterous apo 1 xaraktira kai patisei enter tote sto termch tha paei o 2os char kai oxi to \n(enter)
            //Ara prepei na kathariso to stdin. Den xrhsimopoio fflush giati einai aprovlepti. An dwsei 1 char kai pathsei enter tote
            //sto termch tha paei to \n kai den tha boume sthn if
            if(termch1 != '\n')
                while(getchar() != '\n'); //fflush
