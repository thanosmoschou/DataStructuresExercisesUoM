...
int main()
{
    ListPointer curr, prev,lastSorted, nextFromSorted;
    ListPointer AList;
    NodeType Node[NumberOfNodes];
    ListPointer FreePtr,PredPtr;
    int ctr = 0, tempel[NumberOfNodes];

    CreateList(&AList);
    InitializeStoragePool(Node, &FreePtr);
    //printAll(AList, FreePtr, Node);

    //Until this point i know my list is sorted. From this point i don't know if the rest elements are sorted
    srand(time(NULL)); //Put random numbers because i am lazy :X
    for(int i = 0; i < 4; i++)
        Insert(&AList, Node, &FreePtr, NilValue, rand() % 11);

    for(int i = 15; i >0; i -= 3)
        Insert(&AList, Node, &FreePtr, PredPtr = NilValue, i);
    ...
    while(curr != NilValue)
    {
        if(Node[curr].Data >= Node[Node[curr].Next].Data)
        {
            lastSorted = curr; //I store the position until the sort is valid(including the element that may be bigger than the next one because until these the sort is valid
            nextFromSorted = Node[lastSorted].Next; //the next element from the last sorted...i will store all the remaining values into a table and then i will sort the list
            break;
        }
        else
        {
    ...
    curr = nextFromSorted;
    prev = lastSorted;
    while(curr != NilValue) //store the remaining elements
    {
        tempel[ctr] = Node[curr].Data;
        curr = Node[curr].Next;
        ctr++;
    }

    for(int i = 0; i < ctr; i++) //print the stored elements from the nodes that will be deleted from the list for debugging reasons
        printf("%d ", tempel[i]);
    printf("\n");
    ...
    curr = nextFromSorted;
    prev = lastSorted;
    while(curr != NilValue) //delete the remaining nodes where sort is not valid
    {
        curr = Node[curr].Next;
        Delete(&AList, Node, &FreePtr, prev);
    }

    printf("List without deleted nodes\n");
    TraverseLinked(AList, Node);

    for(int i = 0;  i < ctr; i++) //ctr shows until where i stored elements into the tempel table
    {
        curr = AList;
        prev = NilValue;
        while(curr != NilValue) //very element from the table i am gonna compare it with every element on the list until the
        {                       //condition will not be valid anymore..this is the place where i should place the element and this is the reason i store the position of the previous element
            if(tempel[i] > Node[curr].Data)
            {
                prev = curr;
                curr = Node[curr].Next;
            }
        ...
        Insert(&AList, Node, &FreePtr, prev, tempel[i]);
    }

    printf("List with sorted items(included data in deleted nodes)\n");
    TraverseLinked(AList, Node);
