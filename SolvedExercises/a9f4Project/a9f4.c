...
int main()
{
    ...
    StackType mesStack;
    QueueType mesStack2;
    ListPointer popItem, removeQItem;

    printf("Please enter the number of characters that your string has: ");
    scanf("%d", &sumNum);
    message = (char *)malloc(sumNum * sizeof(char));

    printf("Please enter a string(Do not put spaces): ");
    scanf("%s", message);
    //message = "kalimera";

   	CreateList(&AList);
	CreateStack(&mesStack);
	CreateQ(&mesStack2);


	while(message[ctr] != '\0')
    {
        LinkedInsert(&AList, message[ctr], NULL);
        Push(&mesStack,  AList);
        AddQ(&mesStack2, AList);
        ctr++;
    }

    printf("-------Stack of characters-------\n");
    TraverseStack(mesStack);
    ...
    for(i = 0; i < ctr; i++)
    {
        Pop(&mesStack, &popItem);
        RemoveQ(&mesStack2, &removeQItem);

        if(popItem->Data != removeQItem->Data)
        {
            palindrome = FALSE;
            break;
        }
...
void TraverseQ(QueueType Queue) {
	...
	if (!EmptyQ(Queue)) {
	    ...
	    while (current != Queue.Rear) {
            printf("%c ", Queue.Element[current]->Data);
