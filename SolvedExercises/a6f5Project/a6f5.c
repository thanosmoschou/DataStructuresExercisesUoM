...
int main()
{
    ...
    char word[LIMLETT] = {'\0'}, termch;

    printf("Please insert your word(Max %d characters): ", LIMLETT);
    scanf("%24[^\n]%c", word, &termch);
...
int BSTDepth(BinTreePointer Root, char word[LIMLETT])
{
    int maxlevel, level;
    BinTreePointer locPtr;
    ...
    for(int i = 0; word[i] != '\0'; i++)
    {
        ...
        while(locPtr != NULL)
        {
            if(word[i] < locPtr->Data)
                locPtr = locPtr->LChild;
            else if(word[i] > locPtr->Data)
                locPtr = locPtr->RChild;
            else
                break;
