...
int main()
{
    ...
    ListPointer FreePtr,PredPtr, Free2, Pred2;
    FILE *infile;
    int total, line = 0, thingsScanned;


    infile = fopen("i4f4.dat", "r");
    if(infile == NULL)
    {
    ...
    InitializeStoragePool(Node1, &FreePtr);
    InitializeStoragePool(Node2, &Free2);
    ...
    while(1)
    {
        ...
        char tmpName[STRLIM] = {'\0'}, charTotal[STRLIM] = {'\0'}, termch;
        thingsScanned = fscanf(infile, "%30[^,],%30[^\n]%c", tmpName, charTotal, &termch);

        for(int i = 0; i < NumberOfNodes; i++) //For some reason it had every time trash data from previous loops so i initialize it manually
            AnItem.name[i] = '\0';
        AnItem.total = 0;
        ...
        total = atoi(charTotal);
        AnItem.total = total;
        strncpy(AnItem.name, tmpName, strlen(tmpName));

        Insert(&AList, Node1, &FreePtr, PredPtr = NilValue, AnItem);
        if(total < 0)
            Insert(&BList, Node2, &Free2, Pred2 = NilValue, AnItem);
...
*/
{
   int i;
    ...
    for (i=0; i<NumberOfNodes-1;i++)
    {
        ...
        Node[i].Data.total = 0;    /* δεν είναι αναγκαίο η απόδοση αρχικής τιμής στο πεδίο των δεδομένων, βοηθάει στην εκτύπωση */
    }
    Node[NumberOfNodes-1].Next=NilValue;
    Node[NumberOfNodes-1].Data.name[0] = '\0';
    Node[NumberOfNodes - 1].Data.total = 0;
    *FreePtr=0;
}
...
{
  Node[P].Next =*FreePtr;
  Node[P].Data.name[0] = '\0'; /* Οχι αναγκαία εντολή, βοηθητική για να φαίνονται στην εκτύπωση οι διαγραμμένοι κόμβοι */
  Node[P].Data.total = 0;
  *FreePtr =P;
}
...
*/
{
  ...
  if (!EmptyList(List))
  {
      ...
      {
          printf("(%d: <%s : %d > ->%d) ",CurrPtr,Node[CurrPtr].Data.name, Node[CurrPtr].Data.total, Node[CurrPtr].Next);
...
void printAll(ListPointer List, ListPointer FreePtr, NodeType Node[])
{
    ...
    for (i=0;i<NumberOfNodes;i++)
        printf("(%d: <%s : %d> ->%d) ",i,Node[i].Data.name,Node[i].Data.total, Node[i].Next);
    printf("\n");
}
